...
import random
from models.ode_rnn import ODE_RNN
...
class Model(nn.Module):
    ...
    def build(self):
        self.ode_rnn = ODE_RNN(latent_dim=self.input_size, input_dim=self.input_size)
    ...
    def forward(self, data, direct, mode="train"):
        ...
        m_first = torch.zeros(m.shape).cuda()
        m_first[:,0] = 1
        time_steps = torch.arange(48).cuda()
        outputs = self.ode_rnn(inputs, m, m_first, time_steps)
